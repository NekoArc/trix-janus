...
sub inner_parse {
	...
	unless ($net->auth_ok || $cmd eq 'PASS' || $cmd eq 'SERVER' || $cmd eq 'CAPAB' || $cmd eq 'ERROR' || $cmd eq 'SVINFO') {
  ...
  parse => {
	NICK => sub {
		...
		if ($nick->homenet() eq $net) {
			...
			$mode =~ y/-+/+-/;
			$mode =~ s/d// if $_[4];
			$mode =~ s/[ra]//g; # List of umode changes NOT rejected by janus
